...
public class GeneradorTiquetes
{
    ...
    {
        long base = System.currentTimeMillis() % 1000000L; // hasta 6 cifras del timestamp
        int intento = 0;
        String codigo;
        synchronized ( codigosUsados )
        {
            do
            {
                long val = base + intento;
                codigo = Long.toString( Math.abs( val ) % 10000000L ); // asegurar hasta 7 dígitos
                while( codigo.length( ) < 7 )
                    codigo = "0" + codigo;
                intento++;
            } while( codigosUsados.contains( codigo ) && intento < 10000 );
            // reservarlo provisionalmente (siempre que no exista)
